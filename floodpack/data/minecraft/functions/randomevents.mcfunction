...
execute as @e[tag=waterLevel] at @s if predicate icechance run function subfunctions/freezeevent

# Item-related spawns
execute as @e[tag=waterLevel] at @s if predicate midgamechestchance unless predicate issecondtrimester unless predicate isthirdtrimester run function chestspawns/spawnmgchests
execute as @e[tag=waterLevel] at @s if predicate lategamechestchance unless predicate isfirsttrimester unless predicate isreceding run function chestspawns/spawnlgchests
...
# Chat events
execute if predicate defmobchance run tellraw @a[limit=3,sort=random] ["",{"selector":"@r"},{"text":" fell from a high place"}]